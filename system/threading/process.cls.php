<?php
    ...
    class Process {
         ...
         * @return Process
         */
        public static function Create(Worker $worker) : Process {
        ...
        public function Run(stdClass $params) {
            $pid = shell_exec('cd '.$_SERVER['DOCUMENT_ROOT'].' && ' . Process::Handler.' index.php '.$_SERVER['HTTP_HOST'].' / worker="'.$this->_worker->Serialize().'" params="'.$this->_worker->PrepareParams($params).'" > /dev/null & echo $!');
            $this->_pid = trim($pid, "\n\r\t ");
        }
